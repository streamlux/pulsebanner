...
handler.put(async (req, res): Promise<void> => {
    ...
    if (profileImage) {
        context.logger.info('Got current profile image');
        const twitterInfo = await AccountsService.getTwitterInfo(userId, true);
        if (!twitterInfo) {
            return res.send(400);
        }
        const currentTwitterProfilePic: string = await getTwitterProfilePic(context, twitterInfo.oauth_token, twitterInfo.oauth_token_secret, twitterInfo.providerAccountId);
        context.logger.info('Got Twitter info and current Twitter profile picture.');
        ...
        context.logger.info('Finished refreshing Twitter profile picture.');
